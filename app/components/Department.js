...
export default function Department() {
  return (
    <div className="bg-blue-50 min-h-auto flex justify-center items-center">
      <div className="relative flex justify-center items-center min-h-[60vh] w-full">
        <div className="absolute top-8 bg-blue-50 rounded-lg p-8 w-full max-w-screen-xl ">
          <h1 className="text-5xl font-bold text-center text-blue-900 mb-8 flex justify-center items-center gap-2">
            <Hospital className="w-9 h-9 text-blue-900" /> Departments
          </h1>
          <Swiper
            slidesPerView={1}
            spaceBetween={10}
          ...
          >
            {departments.map((dept, index) => (
              <SwiperSlide key={index} className="bg-white rounded-lg shadow-md p-6 text-center ">
                <div className='hover:cursor-pointer hover:scale-[1.1] transition duration-300'>

                <img alt={`${dept.name} icon`} className="mx-auto mb-4 rounded-lg" height="100" src={dept.img} width="100" />
